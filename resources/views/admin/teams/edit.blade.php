<x-layouts.admin :title="'Edit Team: ' . $team->name">
    ...
    <form method="POST" action="{{ route('admin.teams.update', $team->teamId) }}" style="max-width:480px;">
        @csrf @method('PUT')
        @include('admin.teams._form', ['games' => $games, 'selectedGame' => $team->game, 'team' => $team])
        <div style="display:flex; gap:8px; margin-top:20px;">
            <button type="submit" class="hlx-btn-gold">Save</button>
            <a href="{{ route('admin.teams.index', ['game' => $team->game]) }}" class="hlx-btn-green">Cancel</a>
        </div>
    </form>
</x-layouts.admin>
