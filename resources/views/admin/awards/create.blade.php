<x-layouts.admin title="Add Award">
    ...
    <form method="POST" action="{{ route('admin.awards.store') }}" style="max-width:480px;">
        @csrf
        @include('admin.awards._form', ['games' => $games, 'selectedGame' => $selectedGame])
        <div style="display:flex; gap:8px; margin-top:20px;">
            <button type="submit" class="hlx-btn-gold">Create</button>
            <a href="{{ route('admin.awards.index', ['game' => $selectedGame, 'type' => $selectedType ?? '']) }}" class="hlx-btn-green">Cancel</a>
        </div>
    </form>
</x-layouts.admin>
